...
5. Forwards all other DNS queries unchanged to Google Public DNS (8.8.8.8) — the
   hardcoded upstream in `DnsVpnService.UPSTREAM_DNS_HOST`; queries are NOT sent to
   the device's configured resolvers while the monitor is active
...
> NXDOMAIN, and all other queries are forwarded unchanged to Google Public DNS
> (8.8.8.8). There is no remote VPN server: the tunnel terminates inside the
...
> forwarded to a public resolver (8.8.8.8) exactly as the operating system
> would send them without the VPN.
...
> تُمرَّر إلى خادم Google Public DNS ‏(8.8.8.8) للاستجابة المعتادة، وتُسجَّل
