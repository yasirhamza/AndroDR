...
**Triage every row this prints, don't just diff against the seed.** Any OEM/AOSP/GOOGLE-category keyboard that is not an exclude-family package (Step 3) and not already on the seed is a stock IME to add *now* rather than leave to fire a `low` on the devices that ship it — the DB already contains several the seed omits, e.g. `com.android.inputmethod.pinyin` (OEM), `com.google.android.vr.inputmethod` (GOOGLE), and the Sony `com.sonymobile.android.externalkeyboard` / `…externalkeyboardjp` pair (OEM). Confirm each actually declares an input-method service before adding it (an inert non-IME entry is harmless but pointless). Exclude-family packages stay off regardless of category.
