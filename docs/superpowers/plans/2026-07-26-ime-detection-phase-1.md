...
- **The allowlist is a name-only exemption, not a trust boundary.** `matchEquals` lowercases both sides (`SigmaRuleEvaluator.kt:304`), so the match is case-*insensitive* and binds to no signature. A sideloaded IME can impersonate an allowlisted package name (or a case variant) on any device where that name is free and be silenced — the ADR #51 bypass `androdr-089`/`androdr-011` guard against with `from_trusted_store: true`, which this rule cannot borrow (it would false-positive on sideloaded FOSS keyboards). Phase 1 ships this knowingly: for a FOSS/USER_APP name outside the OEM namespaces the app still trips `androdr-014`, but a name that is an OEM/AOSP/GOOGLE DB entry **or** in a `com.google.`/`com.android.` namespace gets `is_known_oem_app: true` (DB category **or** an unconditional OEM prefix, `AppScanner.kt:255`) and evades `androdr-010/011/014` too — invisible app-wide. That app-wide gap is the pre-existing #263 conflation, which `androdr-090` neither creates nor widens; cert-binding (a keyed `(package, certHash)` lookup — a larger capability than §5's flat allowlist lookup, not a field clause) is the only real fix and is Phase 2. See spec §4. Do **not** "harden" the rule by re-adding a provenance clause without reading that section.
...
**Triage every row this prints, don't just diff against the seed.** Any OEM/AOSP/GOOGLE-category keyboard that is not an exclude-family package (Step 3) and not already on the seed is a stock IME to add *now* rather than leave to fire a `low` on the devices that ship it — the DB already contains several the seed omits, e.g. `com.android.inputmethod.pinyin` (AOSP), `com.google.android.vr.inputmethod` (GOOGLE), and the Sony `com.sonymobile.android.externalkeyboard` / `…externalkeyboardjp` pair (OEM). Confirm each actually declares an input-method service before adding it (an inert non-IME entry is harmless but pointless). Exclude-family packages stay off regardless of category.
